...
(function () {
  ...
  function setLoading(on) {
    if (on) {
      ...
      formWrapper.innerHTML = '<div class="flex flex-col items-center py-4">' +
        ...
        '<p class="mt-4 text-text-secondary text-lg">Checking DNS records...</p>' +
        '</div>';
  ...
  // Grade colors — 13 grades (same as headers checker)
  var gradeColors = {
  ...
  var testNames = {
    ...
    'caa': 'CAA Records',
    'ns': 'Nameserver Redundancy',
    'soa': 'SOA Configuration',
    'spf': 'SPF Record',
    'dmarc': 'DMARC Record',
    'mx': 'MX Records',
    'dangling-cname': 'Dangling CNAME'
  };
  ...
  var learnAnchors = {
    ...
    'dmarc': '#learn-dmarc',
    'mx': '#learn-mx',
    'dangling-cname': '#learn-cname'
  };

  var testOrder = ['dnssec', 'spf', 'dmarc', 'caa', 'mx', 'ns', 'soa', 'dangling-cname'];
  ...
  var HISTORY_KEY = 'dnsScanHistory';
  var MAX_HISTORY = 50;
  ...
  function renderRawRecords(rawRecords) {
    var pre = document.getElementById('raw-records-pre');
    if (!pre || !rawRecords) return;
    var lines = [];
    var types = ['A', 'AAAA', 'NS', 'MX', 'TXT', 'CAA', 'SOA', 'CNAME'];
    for (var t = 0; t < types.length; t++) {
      var type = types[t];
      var records = rawRecords[type];
      if (records && records.length > 0) {
        lines.push('<span style="color: #93c5fd; font-weight: 600;">; ' + type + ' Records</span>');
        for (var i = 0; i < records.length; i++) {
          var r = records[i];
          lines.push(
            '<span style="color: #d1d5db;">' + escapeHtml(r.name) + '</span>' +
            '<span style="color: #6b7280;">  ' + escapeHtml(String(r.ttl)) + '  IN  </span>' +
            '<span style="color: #93c5fd;">' + type + '</span>' +
            '<span style="color: #6b7280;">  </span>' +
            '<span style="color: #86efac;">' + escapeHtml(r.data) + '</span>'
          );
        }
        lines.push('');
      }
    ...
    if (lines.length === 0) {
      lines.push('<span style="color: #6b7280;">No DNS records found.</span>');
    }
  ...
  function renderResults(data) {
    ...
    // ── Raw Records tab ──
    if (data.rawRecords) {
      renderRawRecords(data.rawRecords);
    }
  ...
  function doScan(domainInput) {
    ...
    // Fetch challenge token, then check DNS
    ...
    .then(function (tokenData) {
      return fetch(API_BASE + '/api/dns-check', {
        method: 'POST',
  ...
  // Copy records button
  var copyBtn = document.getElementById('copy-records-btn');
  if (copyBtn) {
    copyBtn.addEventListener('click', function () {
      if (lastScanData && lastScanData.rawRecords) {
        var text = '';
        var types = ['A', 'AAAA', 'NS', 'MX', 'TXT', 'CAA', 'SOA', 'CNAME'];
        for (var t = 0; t < types.length; t++) {
          var type = types[t];
          var records = lastScanData.rawRecords[type];
          if (records && records.length > 0) {
            text += '; ' + type + ' Records\n';
            for (var i = 0; i < records.length; i++) {
              var r = records[i];
              text += r.name + '  ' + r.ttl + '  IN  ' + type + '  ' + r.data + '\n';
            }
            text += '\n';
          }
        }
